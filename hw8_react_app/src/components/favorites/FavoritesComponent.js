    ...
    render() {
        if (this.state.articleList !== undefined && this.state.articleList !== null && this.state.articleList.length > 0) {
            var listOfNews = this.state.articleList.map((newsItemId) => {
                ...
                return <Col md={6} lg={3} key={newsItem.id} id={newsItem.id}><FavoriteCardComponent newsObj = {newsItem} handleDeleteFromFavorites={this.handleDeleteFromFavorites} /></Col>
            })
        }
        return (
            <>
